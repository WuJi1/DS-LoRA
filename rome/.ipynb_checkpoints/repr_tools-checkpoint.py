...
def get_reprs_at_word_tokens(
    ...
    """
    Retrieves the representation of `word` in `context_template`
...
def get_words_idxs_in_templates(
    ...
    """
    Given list of template strings, each with one format specifier "{}",
    and words to be substituted into the template, computes the token index
    corresponding to the selected subtoken of each word.
    """
...
def get_reprs_at_idxs(
    ...
    Memory-safe modification:
        The extracted token representations are immediately moved to CPU.
        This avoids CUDA OOM when thousands of representations are stacked.
    """
    ...
    def _process(cur_repr, batch_idxs, key):
        """
        Extract selected token representations.

        Important:
            Each selected representation is immediately moved to CPU.
            Otherwise, all representations are kept on GPU and the final
            torch.stack(v, 0) may require a huge contiguous CUDA allocation.
        """
        nonlocal to_return

        cur_repr = cur_repr[0] if type(cur_repr) is tuple else cur_repr

        # Some models return [seq_len, batch, hidden] instead of [batch, seq_len, hidden]
        if cur_repr.shape[0] != len(batch_idxs):
            cur_repr = cur_repr.transpose(0, 1)

        for i, idx_list in enumerate(batch_idxs):
            token_repr = cur_repr[i][idx_list].mean(0)
            to_return[key].append(token_repr.detach().cpu())
    ...
    for batch_contexts, batch_idxs in _batch(n=128):
        ...
        # Release cached CUDA blocks between mini-batches.
        # This is especially useful for 20000-edit runs.
        if torch.cuda.is_available():
            torch.cuda.empty_cache()

    # Now v contains CPU tensors, so stack is performed on CPU.
    to_return = {
        k: torch.stack(v, 0)
