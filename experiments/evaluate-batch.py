...
from dslora.lora_main_batch import apply_dslora_to_model 
...
import debugpy
# debugpy.listen(("127.0.0.1", 5678))  # 服务器开放端口
# print("Waiting for debugger attach...")
# # debugpy.wait_for_client()          # 可选：阻塞等待
# debugpy.breakpoint()               # 程序在这里停住
import random
import numpy as np
import torch
    ...
    if any(alg in alg_name for alg in ["AlphaEdit", "MEMIT_seq", "MEMIT_prune", "NSE", "LoRA","DSLoRA","LoRA"]):
        ...
        if hparams.model_name == "gpt2-xl":
            ...
            if alg_name == "AlphaEdit":
                P = torch.zeros((len(hparams.SClayers), W_out.shape[0], W_out.shape[0]), device="cpu")
            if alg_name == "DSLoRA":
...
def get_project(model, tok, layer, hparams):
    ...
    cov = get_cov(
        ...
        hparams.mom2_dtype,
        force_recompute=force_recompute,
    ) # 保持在原始设备，通常 get_cov 内部如果是从 GPU 收集的，这里就在 GPU
    ...
    # 3. 在 GPU 上执行 SVD
    # 注意：对于对称正定矩阵，使用 eigh (特征值分解) 通常比 svd 更快且更稳定
    try:
        # 使用 SVD 逻辑
        U, S, _ = torch.linalg.svd(cov, full_matrices=False)
        ...
        # 4. 计算投影矩阵并返回
        # 结果可以保持在 GPU 用于后续计算，或者 .cpu() 返回减少显存占用
        U_small = U[:, small_singular_indices]
        project_mat = U_small @ U_small.T
        
        return project_mat.cpu() # 建议返回 CPU 版，避免长期占用显存
...
# # 1. 查看当前设定阈值的能量占比
# curr_indices = (S < 0.0005).nonzero(as_tuple=True)[0]
# curr_ratio = S[curr_indices].sum() / total_energy
# print(f"Layer {layer} | Thres {hparams.nullspace_threshold} | Dims {len(curr_indices)} | Ratio: {curr_ratio.item():.5%}")
