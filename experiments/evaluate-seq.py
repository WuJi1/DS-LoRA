...
from dslora.lora_main import apply_dslora_to_model 
from dslora import DSLoRAHyperParams
from lora.lora_main import apply_lora_to_model 
...
ALG_DICT = {
    ...
    "DSLoRA": (DSLoRAHyperParams, apply_dslora_to_model)
}
...
DATASET_CONFIG = {
    "zsre": {
        "total": 19086,
        "save_steps": [2000,4000,5000,6000,8000,10000]
    },
    "cf": {
        "total": 21919,
    ...
    "mcf": {
        "total": 20877,
        "save_steps": [2000,4000,5000,6000,8000,10000,15000,20000]
    },
}
...
def main(
    ...
    if type(model_name) is str:
        ...
        elif model_name == "EleutherAI_gpt-j-6B":
    ...
    if any(alg in alg_name for alg in ["AlphaEdit", "MEMIT_seq", "MEMIT_prune", "NSE", "LoRA","DSLoRA","LoRA"]):
        ...
        if hparams.model_name == "gpt2-xl":
            ...
            if alg_name == "AlphaEdit":
                P = torch.zeros((len(hparams.DSlayers), W_out.shape[0], W_out.shape[0]), device="cpu")
            if alg_name == "DSLoRA":
                P = torch.zeros((len(hparams.layers), W_out.shape[0], W_out.shape[0]), device="cpu")
        elif hparams.model_name in ["EleutherAI_gpt-j-6B","Llama3-8B","Llama32-3B","phi-1.5","Qwen3-8B","Qwen2.5-7B","Meditron-7B"]:
        # elif "Llama" in hparams.model_name.lower() or "Qwen" in hparams.model_name.lower():
            if alg_name == "DSLoRA":
                cache_c = None
            else:
            ...
            if alg_name == "DSLoRA":
                # P = torch.zeros((hparams.total_layers, W_out.shape[1], lora_r), device="cpu")
                P = torch.zeros((len(hparams.layers), W_out.shape[1], W_out.shape[1]), device="cpu")
        del W_out
    ...
    if alg_name == "AlphaEdit":
        ...
        if os.path.exists(f"{model_name}-{alg_name}-null_space_project.pt"):
            # Load the existing null space project
            P = torch.load(f"{model_name}-{alg_name}-null_space_project.pt")
        else:
            # Generate and save the null space project
            for i, layer in enumerate(hparams.layers):
                P[i, :, :] = get_project(model, tok, layer, hparams)
            torch.save(P, f"{model_name}-{alg_name}-null_space_project.pt")
            
    if alg_name == "DSLoRA": 
        if os.path.exists(f"{model_name}-{alg_name}-null_space_project-{hparams.nullspace_threshold}.pt"):
            # Load the existing null space project
            P = torch.load(f"{model_name}-{alg_name}-null_space_project-{hparams.nullspace_threshold}.pt")
            P_cov = torch.zeros_like(P, device="cpu")
            for i, layer in enumerate(hparams.layers):
                P_cov[i, :, :] = get_P_cov(model, tok, layer, hparams)
        else:
            # Generate and save the null space project
            P_cov = torch.zeros_like(P, device="cpu")
            for i, layer in enumerate(hparams.layers):
                P[i, :, :] = get_project(model, tok, layer, hparams)
                P_cov[i, :, :] = get_P_cov(model, tok, layer, hparams)
            torch.save(P, f"{model_name}-{alg_name}-null_space_project-{hparams.nullspace_threshold}.pt")       
    ...
    for record_chunks in chunks(ds, num_edits):
        ...
        seq_args = dict(cache_c=cache_c) if any(alg in alg_name for alg in ["AlphaEdit", "MEMIT_seq", "NSE","DSLoRA"]) else dict()
        nc_args = dict(P = P) if any(alg in alg_name for alg in ["AlphaEdit","DSLoRA"]) else dict()
        cov_args = dict(P_cov = P_cov) if any(alg in alg_name for alg in ["DSLoRA"]) else dict()
        ...
        start = time()
        if any(alg in alg_name for alg in ["AlphaEdit", "MEMIT_seq", "NSE", "LoRA","DSLoRA"]):
        ...
        cur_step = cnt * num_edits
        ds_key = ds_name.lower()
        

        if ds_key in DATASET_CONFIG and cur_step in DATASET_CONFIG[ds_key]["save_steps"]:

            checkpoint_dir = f"./Edited_Weight/{alg_name}/{model_name.replace('/', '_')}/{ds_name}/"
            ...
            with torch.no_grad():
                for name, params in model.named_parameters():
                    ...
                    if any(hparams.rewrite_module_tmp.format(layer) in name for layer in hparams.layers):
                        weights_to_save[name] = params.detach().cpu()
            
            save_file = os.path.join(checkpoint_dir, f"edit_{cur_step}.pth")
            torch.save({"weight": weights_to_save}, save_file)
            print(f"✅ Checkpoint saved at step {cur_step}: {save_file}")
...
def get_project(model, tok, layer, hparams):
    ...
    try:
        ...
        print(f"Layer {layer} - Nullspace dims: {len(small_singular_indices)}")
        U_small = U[:, small_singular_indices]
        project_mat = U_small @ U_small.T
        
        return project_mat.cpu() 
        
    except RuntimeError as e:
        if "out of memory" in str(e):
            print("⚠️ GPU OOM during SVD, falling back to CPU...")
            torch.cuda.empty_cache()
            return get_project_cpu_fallback(cov.cpu(), hparams)
        raise e

def get_P_cov(model, tok, layer, hparams):
    force_recompute = False
    cov = get_cov(
        model,
        tok,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--alg_name",
        choices=["AlphaEdit","MEMIT_rect", "MEMIT_seq","MEMIT_prune", "MEMIT", "ROME", "FT", "MEND","NSE","DSLoRA","LoRA"],
        default="ROME",
